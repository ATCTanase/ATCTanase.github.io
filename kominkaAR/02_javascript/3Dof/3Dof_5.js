...
// ARアニメ
const ARImage1 = "../../04_image/ARImage/AR5_お母さん";
const ARImage2 = "../../04_image/ARImage/AR5_お父さん";
const ARImage3 = "../../04_image/ARImage/AR5_お客さん";
const frameCount = 1;
const frameExt = ".png";
const frames1 = [];
const frames2 = [];
...
let markerPositionX2 = 0;
let markerPositionY2 = 1;
let markerPositionZ2 = -2;

let markerRotationX2 = 0;
let markerRotationY2 = 0;
let markerRotationZ2 = 0;

let markerHeight2 = 3;
let markerWidth2 = 3;

let markerPositionX3 = 2;
let markerPositionY3 = 0;
let markerPositionZ3 = 0;

let markerRotationX3 = 0;
let markerRotationY3 = 0;
let markerRotationZ3 = 0;
  ...
  for (let i = 1; i <= frameCount; i++) {
    ...
    img3.onload = () => {
      ...
      if (loaded === frameCount) {
        ...
        markerHeight3 = markerWidth3 * offset3;
      }
    };
    frames3.push(img3);
  }
...
function updateVideoPlane() {
  if (markerVisible) {
    ...
    const offsetPosition3 = new THREE.Vector3(
      Number(markerPositionX3),
      Number(markerPositionY3),
      Number(markerPositionZ3)
    );
    // マーカーのワールド回転を適用
    const rotatedOffset3 = offsetPosition3.clone().applyQuaternion(stableMarkerWorldQuat);
    const finalPos3 = markerWorldPos.clone().add(rotatedOffset3);

    // --- 回転補正 ---
    const offsetEuler3 = new THREE.Euler(
      THREE.MathUtils.degToRad(markerRotationX3),
      THREE.MathUtils.degToRad(markerRotationY3),
      THREE.MathUtils.degToRad(markerRotationZ3)
    );
    const offsetQuat3 = new THREE.Quaternion().setFromEuler(offsetEuler3);
    const finalQuat3 = stableMarkerWorldQuat.clone().multiply(offsetQuat3);

    // --- スケール補正 ---
    const markerScale3 = barcodeMarker.object3D.scale.clone();
    const offsetScale3 = new THREE.Vector3(markerWidth3, markerHeight3, 1);
    const finalScale3 = markerScale3.multiply(offsetScale3);

    // --- 適用 ---
    videoPlane3.object3D.position.copy(finalPos3);
    videoPlane3.object3D.quaternion.copy(finalQuat3);
    videoPlane3.object3D.scale.copy(finalScale3);
  }
}
...
videoPlane3.addEventListener("loaded", () => {
  ...
  array.forEach(function (value) {
    ...
    if (content[0].trim(" ") == "arPosition5_3") {
      ...
      markerPositionX3 = arPosition[0];
      markerPositionY3 = arPosition[1];
      markerPositionZ3 = arPosition[2];
    }

    if (content[0].trim(" ") == "arRotation5_3") {
      const arRotation = content[1].split(",");

      markerRotationX3 = arRotation[0];
      markerRotationY3 = arRotation[1];
      markerRotationZ3 = arRotation[2];
    }

    if (content[0].trim(" ") == "arHeightWidth5_3") {
      const arHeightWidth = content[1].split(",");

      markerHeight3 = Number(arHeightWidth[0]);
      markerWidth3 = Number(arHeightWidth[1]);
    }
  })
});
...
function openDialog3() {
  document.getElementById("XDirection").value = markerPositionX3;
  document.getElementById("YDirection").value = markerPositionY3;
  document.getElementById("ZDirection").value = markerPositionZ3;
  document.getElementById("XRotation").value = markerRotationX3;
  document.getElementById("YRotation").value = markerRotationY3;
  document.getElementById("ZRotation").value = markerRotationZ3;

  document.getElementById("height").value = markerHeight3;
  document.getElementById("width").value = markerWidth3;
...
function closeDialog() {
  ...
  if (modalIdentification == 1) {
    ...
    markerRotationX3 = xRotation.value;
    markerRotationY3 = yRotation.value;
    markerRotationZ3 = zRotation.value;

    markerHeight3 = width.value * offset3;
    markerWidth3 = width.value;

    markerPositionX3 = Number(xDirection.value);
    markerPositionY3 = Number(yDirection.value);
    markerPositionZ3 = Number(zDirection.value);
...
function debugScreenFlexGrid(angle) {
    if (angle == 0) {
        ...
        ARTargetImg.setAttribute("src", "../../04_image/ARImage/map_img_ar_frame_vertical.png");
    ...
    if (angle == 90) {
        ...
        ARTargetImg.setAttribute("src", "../../04_image/ARImage/map_img_ar_frame.png");
