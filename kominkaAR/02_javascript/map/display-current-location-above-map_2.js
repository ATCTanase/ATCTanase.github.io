...
navigator.geolocation.watchPosition(
    ...
    function(position){
        ...
        // 34.77948547950274,  135.48723969855405
        // 34.779247137940935, 135.48823541783045
        const latitude  = 34.779247137940935;
        const longitude = 135.48823541783045;
        ...
        // console.log("緯度  ：", ratioY);
        // console.log("経度  ：", ratioX);
        // const ratioY = Math.round(((yMax - y) / (yMax - yMin)) * 495.501);
        // const ratioX = Math.round(((longitude - minX) / (maxX - minX)) * 352.7);
        gpsRatioY = 352.7 - ratioX;
        gpsRatioX = ratioY;
        draw();

        // 地図の範囲内に現在地がいるならマップにマーカーを表示
        // パーセントだと地図を拡縮してもズレない
        // アプリ内の地図は北が左に向いているので、ratioYをtop→left, ratioXをleft→bottomで設定する
        if((minY < latitude && latitude < maxY) && (minX < longitude && longitude < maxX)){
            $("#mapCurrentLocationErea").css({
                left:   `${ratioY}%`,
                bottom: `${ratioX}%`,
            });
        }else{
            $("#mapCurrentLocationErea").css("display", "none");
        }
    },
